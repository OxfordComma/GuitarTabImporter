...
export function formatRawTabs(rawTabs) {
	...
	let chordRowRegex = /^[A-Gb#mj47su]+( |$)/gm
	...
	for (var i = 0; i < rawTabsSplit.length - 1; i++) {
		...
		if (currentRow.match(chordRowRegex)) {
	...
	for (var i = 0; i < rawTabsSplit.length - 1; i++) {
		...
		if (thirdRow.match(chordRowRegex) && fourthRow.match(/^[A-Gb#m7su]+( |$)/gm)) {
			continue
		}

		if (currentRow.match(chordRowRegex) && currentRow.split(' ').filter(i => i != '').length <= 2) {
