...
export function formatRawTabs(rawTabs) {
	...
	let chordRowRegex = /^ *[A-G][b#mj47su]*?( |$)/gm
	...
	for (var i = 0; i < rawTabsSplit.length; i++) {
		...
		let chordCount = thirdRow.split(' ').filter(i => i != '').length

		if (currentRow.match(chordRowRegex) && !nextRow.match(chordRowRegex) && chordCount <= 2) {
